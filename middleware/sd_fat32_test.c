...
int sd_fat32_append_data_test()
{
    char *dir_path = "/MYDIR/TEST";
    char *file_path = "/MYDIR/TEST/test.txt";
    char *data = "Hello FAT32! Appending test data.";
    char buffer[128] = {0};
    int status;

    // 1. Создание директории (если ещё не создана)
    status = mkdir_fat32(dir_path);
    if (status == 0)
    {
        LOG_INFO("Directory created: %s\r\n", dir_path);
    ...
    {
        LOG_INFO("Directory %s may already exist or creation failed (err=%d)\r\n", dir_path, status);
    }

    // 2. Открытие файла для дозаписи
    FAT32_File *file = NULL;
    status = open_file_fat32(file_path, &file, F_APPEND);
    if (status != 0)
    {
        LOG_INFO("Failed to open file %s for append (err=%d)\r\n", file_path, status);
        return status;
    }
    LOG_INFO("File opened for append: %s\r\n", file_path);

    // 3. Дозапись данных в файл
    status = write_file_fat32(file, data, strlen(data));
    if (status < 0)
    {
        LOG_INFO("Failed to append to %s (err=%d)\r\n", file_path, status);
        close_file_fat32(&file);
        return status;
    }

    close_file_fat32(&file);
    LOG_INFO("Data appended to %s\r\n", file_path);

    // 4. Чтение данных обратно для проверки
    status = open_file_fat32(file_path, &file, F_READ);
    if (status != 0)
    {
        LOG_INFO("Failed to open file %s for read (err=%d)\r\n", file_path, status);

...
int is_special_dir(const uint8_t dir_name[11]);
...
int validate_fat_sfn_file(const char *file_name);
int validate_fat_lfn_file(const char *file_name);
int validate_fat_lfn_dir(const char *dir_name);
int validate_fat_sfn_dir(const char *dir_name);
...
int create_file_fat32(uint32_t cluster_directory, uint32_t *cluster_file, char *file_name)
{
    ...
    *cluster_file = FILE_END_TABLE_FAT32;
    ...
    else
    {
        ...
        entry->DIR_Attr = ATTR_ARCHIVE;
        split_cluster_number(*cluster_file, &entry->DIR_FstClusHI, &entry->DIR_FstClusLO);
        format_fat_sfn(file_name, length, (char *)entry->DIR_Name);
    }
...
int create_dir_fat32(char *name, uint32_t length, uint32_t parent_cluster)
{
    ...
    else
    {
        ...
        split_cluster_number(cluster_new_dir, &entry->DIR_FstClusHI, &entry->DIR_FstClusLO);
        format_fat_sfn(name, length, (char *)entry->DIR_Name);
    }
...
int compare_lfn(char *name_ascii, uint16_t *name_unicode)
{
    ...
    utf16le_to_ascii(name_unicode, (char *)buffer);
...
void print_char(char *text, int size)
{
    if (text == NULL)
    {
        LOG_INFO("Error: NULL pointer encountered\n");
        return;
    }
...
{
    LOG_INFO("FSInfo:");
    LOG_INFO("\tFSI_Nxt_Free: %d, FSI_Free_Count: %d\n", data->FSI_Nxt_Free, data->FSI_Free_Count);
}
...
void show_lentry_data(LDIR_Type *entry)
{
    ...
    LOG_INFO("LDIR_Ord: %d,LDIR_Attr: %x, LDIR_Chksum: %d, LDIR_FstClusLO: %d, LDIR_Type: %d\n", entry->LDIR_Ord, entry->LDIR_Attr, entry->LDIR_Chksum,
...
int clear_table_fat32()
{
    ...
    uint32_t *buffer = pool_alloc(sector_size);
...
cleanup:
    if (pool_free_region(buffer, sector_size) != 0)
    {
        // Добавить в лог
    }
...
{
    uint32_t buffer[128];
    LOG_INFO("table 1 sector: %d\n", sector);
    fat_info->device->read((uint8_t *)buffer, 512, sector);
    for (int i = 0; i < 128; ++i)
    {
        LOG_INFO("cluster %d: %x ", i, buffer[i]);
    }
    puts("");
}